...
def find_cheapest_tree(root, k, s):
    ...
    def dfs(node, depth=0):
        ...
        if node.ancestor:
            ...
            if node.ancestor.score == 0:
                # sort leaf nodes here
        ...
        if len(local_list) >= k:
            #finds first common node between the ancestor of the node and nodes existing in the local_list
            for n in leaf_nodes.copy():         #Error: dictionary keys changed during iteration
                ...
                while common_anc.name not in local_list:
                ...
                # cat_Personality: personality (hs a original score value)
                ...
                if (dp[leaf.num] - dp[leaf_anc.num]) >= (dp[node.num] - dp[path1[0].num]):
                    ...
                    print(f"--Added '{node.name}', Removed {path2}--")

                    # check children of path2 nodes not existing in local_list
                    t = False
                    for c in path2:
                        d = local_list[c]
                        if any(x.name in local_list and x.name not in path2 for x in d.children):
                            # print("fuck this shot")
                            t=True
                            
                    if t:
                        print(f" !!{c} is an ancestor and being removed at the same time")
                        continue

                    # Update leaf_nodes
                    a = local_list[path2[0]].ancestor

                    # delete nodes (path2)
                    c = leaf
                    for _ in range(len(path2)):
                        del local_list[c.name]
                        c = c.ancestor
                    ...
                    if a:
                        if a.score != 0 and not any(x.name in local_list for x in a.children):
    ...
    # change to sort for dfs(root.children) or each cat individually
    # and form a local_list and leaf_nodes for each cat_ 
    # save local_list to a dynamic dictionary
    
    for cat_ in root.children:
        print("\n------")
        print(cat_.name)
        local_list = {}
        leaf_nodes = {}
        tree = dfs(cat_)
        trees_list[cat_.name] = tree
        print_dict(tree)
